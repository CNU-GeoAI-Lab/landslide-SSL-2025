...
def main():
    ...
    parser.add_argument("--pre_batch_size", type = int, default = 256, help = "batch size of the pretraining model")
    ...
    parser.add_argument("--temperature", type = float, default = 0.05, help = "temperature for moco loss")
    ...
    parser.add_argument("--model_type", type = str, default = "ResNet", help = "['ResNet', 'CNN', 'ViT'] sort of pre-training model")
    parser.add_argument("--ssl_type", type = str, default = "SimCLR", help = "['SimCLR', 'BYOL', 'MoCo', 'SimSiam', 'no_pretrain'] sort of self-supervised learning model")
    ...
    if args.ssl_type != "no_pretrain":
        ...
        if args.ssl_type == "BYOL":
            with tf.device('/GPU:0'):
                pretrain_model = pre_train_byol(data_1, data_2, batch_size=args.pre_batch_size, pre_model=args.model_type, epochs=args.pre_epochs)          
        elif args.ssl_type == "SimCLR":
            with tf.device('/GPU:0'):
                pretrain_model = pre_train_simclr(data_1, data_2, batch_size=args.pre_batch_size, pre_model=args.model_type, epochs=args.pre_epochs)
        elif args.ssl_type == "MoCo":
            with tf.device('/GPU:0'):
                pretrain_model = pre_train_moco(data_1, data_2, batch_size=args.pre_batch_size, pre_model=args.model_type, epochs=args.pre_epochs, temperature = args.temperature)
        elif args.ssl_type == "SimSiam":
            with tf.device('/GPU:0'):
                pretrain_model = pre_train_simsiam(data_1, data_2, batch_size=args.pre_batch_size, pre_model=args.model_type, epochs=args.pre_epochs)
    ...
    ls_patches_features = np.load("./data/train/ls_patches.npy")
    non_ls_patches_features = np.load("./data/train/non_ls_patches.npy")
    ls_patches_labels = np.load("./data/train/ls.npy")
    non_ls_patches_labels = np.load("./data/train/non_ls.npy")
    
    ls_test_patches = np.load("./data/test/ls_patches.npy")
    non_ls_test_patches = np.load("./data/test/non_ls_patches.npy")
    ls_test = np.load("./data/test/ls.npy")
    non_ls_test = np.load("./data/test/non_ls.npy")
...
#    valid_ls_label = valid_ls_label[:int(len(valid_ls_ind)*args.fine_tuning_data_ratio)]
    ...
    try:
        os.makedirs('./%s/%s'%(model_name, args.dir_name))
    except:
        print("Directory name %s"%args.dir_name, 'exist')

    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint("./%s/%s/%s.h5"%(model_name, args.dir_name, args.fine_trained_model_name), save_best_only=True)
    checkpoint_cb2 = tf.keras.callbacks.ModelCheckpoint("./%s/%s/%s_weight.h5"%(model_name, args.dir_name, args.fine_trained_model_name), save_weight_only=True)
    ...
    train_ls_coor = np.load('./data/train/ls_coor.npy')
    train_non_ls_coor = np.load('./data/train/non_ls_coor.npy')
    test_ls_coor = np.load('./data/test/ls_coor.npy')
    test_ls_non_coor = np.load('./data/test/non_ls_coor.npy')
    ...
    #test_patches_scaled = np.resize(test_patches_scaled, (test_patches_scaled.shape[0], 18, 18, 20))
    ...
    #np.save('./performance/%s/%s_kappa.npy'%(args.fine_trained_model_name), kp_score)
    #np.save('./performance/%s/%s_f1.npy'%(args.fine_trained_model_name), f1)
    #np.save('./performance/%s/%s_auc.npy'%(args.fine_trained_model_name), auc_score)
